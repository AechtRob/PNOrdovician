...
public class SkyRendererOrdovician extends IRenderHandler {
    ...
    public static final ResourceLocation RING_TEXTURES = new ResourceLocation(LepidodendronMod.MODID + ":textures/environment/ring.png");
	...
	//One Ring to rule them all...
	{
		...
		Vec3d vec = new Vec3d(dx-dx, -12 , dz-4-dz);
		double l = Math.min(Minecraft.getMinecraft().gameSettings.renderDistanceChunks*16, vec.length());
		vec = vec.normalize();
		Vec3d vecd = new Vec3d(vec.x*l, vec.y*l, vec.z*l);
		GL11.glTranslated( vecd.x, vecd.y , vecd.z);
		GL11.glScaled(40, 40, 40);
		renderRing(RING_TEXTURES, partialTicks);
		GL11.glDisable(GL11.GL_TEXTURE_2D);
		GL11.glEnable(GL11.GL_FOG);
		GL11.glPopMatrix();
	}
	...
	public static void renderRing(ResourceLocation loc1, float partialTicks) {
		GL11.glPushMatrix();
		
		GL11.glEnable(GL11.GL_TEXTURE_2D);
		GL11.glEnable(GL11.GL_BLEND);
		GL11.glRotatef(-10, 1.0F, 0.0F, 0.0F);
		GL11.glRotatef(182-(Minecraft.getMinecraft().world.getCelestialAngle(partialTicks)*-360), 0.0F, 0.0F, 1.0F);
        ...
        double ringVisibility = (1-Minecraft.getMinecraft().world.rainingStrength)*Minecraft.getMinecraft().world.getStarBrightness(partialTicks);
		GL11.glColor4d(0.5, 0.5, 0.5, ringVisibility);
		Tessellator tess = Tessellator.getInstance();
		TextureManager tex = Minecraft.getMinecraft().getTextureManager();
		BufferBuilder buff = tess.getBuffer();
		buff.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX);//.startDrawingQuads();
		
		buff.pos(0.0F - f5, 0.0F - f6, 0.0D).tex(1, 0).endVertex();
		buff.pos(f4 - f5, 0.0F - f6, 0.0D).tex(0, 0).endVertex();
		buff.pos(f4 - f5, f4 - f6, 0.0D).tex(0, 1).endVertex();
		buff.pos(0.0F - f5, f4 - f6, 0.0D).tex(1, 1).endVertex();
