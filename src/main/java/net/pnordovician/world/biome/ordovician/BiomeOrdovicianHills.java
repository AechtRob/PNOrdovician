...
public class BiomeOrdovicianHills extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:ordovician_hills")
	public static final BiomeGenCustom biome = null;
	public BiomeOrdovicianHills(ElementsLepidodendronMod instance) {
		super(instance, 1591);
	}
	...
	static class BiomeGenCustom extends BiomeOrdovician {
		public BiomeGenCustom() {
			super(new BiomeProperties("Ordovician Hilly Wastes").setRainfall(0.4F).setBaseHeight(1.95F).setHeightVariation(0.4F).setTemperature(0.1F));
			setRegistryName("lepidodendron:ordovician_hills");
	    ...
	    public void decorate(World worldIn, Random rand, BlockPos pos)
	    {
			if (net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.ICE)) {
				...
				for (int j = 0; j < i; ++j) {
					...
					ICE_GENERATOR.generate(worldIn, rand, blockpos,90);
					//}
				}
				...
				for (int j = 0; j < i; ++j) {
					...
					int l = rand.nextInt(16) + 8;
					BlockPos blockpos = worldIn.getHeight(pos.add(k, 0, l));
					SNOW_GENERATOR.generate(worldIn, rand, blockpos, 90);
				}
